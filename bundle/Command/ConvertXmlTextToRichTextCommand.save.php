...
EOT
    ...
    {
        $statement = $this->gateway->getFieldRows(['ezxmltext'], $contentId, $contentTypeIds, $offset, $limit);
        while ($row = $statement->fetch(FetchMode::ASSOCIATIVE)) {
    ...
    protected function processFields($dryRun, $contentTypeIds, $checkDuplicateIds, $checkIdValues, OutputInterface $output)
    {
        ...
        $count = $this->gateway->getRowCountOfContentObjectAttributes('ezxmltext', null, $contentTypeIds);
